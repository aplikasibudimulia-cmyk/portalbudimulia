-- ============================================================
-- PATCH: Perbaiki pencocokan username di fungsi fn_login
-- Agar mendukung login dengan username pendek maupun lengkap,
...
DECLARE
  ...
  v_db_hash   TEXT;
BEGIN
  -- 1. Cari akun (case-insensitive & support domain strip)
  SELECT * INTO v_akun
  FROM akun_pengguna
  WHERE (
    LOWER(TRIM(username)) = LOWER(TRIM(p_username))
    OR SPLIT_PART(LOWER(TRIM(username)), '@', 1) = SPLIT_PART(LOWER(TRIM(p_username)), '@', 1)
  )
    AND status = 'aktif'
  LIMIT 1;
  ...
  -- Bandingkan hash hasil crypt dengan hash di database
