...
export default function PiketDashboardSection({ session, activeTa }) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { data: presensiMingguanDB } = await supabase
        .from('presensi_harian')
        .select('*')
      ...
      if (presensiMingguanDB) {
        setPresensiMingguan(presensiMingguanDB)
      }
  ...
  const hadirCount = presensiHariIni.filter(p => p.status === 'H').length
  ...
  const barChartData = useMemo(() => {
    return semuaKelas.map(c => {
      const hadir = presensiHariIni.filter(p => p.kelas === c && (p.status === 'H' || p.status === 'T')).length
      return { kelas: c, hadir }
    })
  }, [semuaKelas, presensiHariIni])
  ...
  const absentees = useMemo(() => {
    return presensiHariIni.filter(p => p.status !== 'H' && p.status !== 'P')
  }, [presensiHariIni])
  ...
  return (
    <div className="animate-fade-in font-sans text-slate-800 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 transition-all hover:-translate-y-1 hover:shadow-md">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 transition-all hover:-translate-y-1 hover:shadow-md relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="col-span-1 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          ...
          {totalPerhatian === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
              ...
              <p className="text-sm font-semibold text-slate-700">Luar Biasa!</p>
              <p className="text-xs text-slate-500 mt-1">Tidak ada siswa bermasalah/absen hari ini.</p>
            </div>
          ) : (
            <>
        ...
        {/* Perhatian Hari Ini (List) */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-slate-800">Perhatian Hari Ini</h3>
            <button className="text-slate-400 hover:text-slate-600">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z"></path></svg>
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3 max-h-[220px]">
            {absentees.length === 0 ? (
              <div className="text-center text-slate-500 py-6 text-sm">Semua siswa hadir atau belum ada data.</div>
            ) : absentees.sort((a,b) => a.kelas.localeCompare(b.kelas)).map((p, idx) => {
              const s = semuaSiswa.find(x => x.nisn === p.siswa_nisn)
              const name = s ? s.nama_lengkap : p.siswa_nisn
              const badges = {
                'T': 'bg-orange-50 text-orange-600',
                'S': 'bg-blue-50 text-blue-600',
                'I': 'bg-purple-50 text-purple-600',
                'A': 'bg-rose-50 text-rose-600'
              }
              const labels = { 'T': 'Terlambat', 'S': 'Sakit', 'I': 'Izin', 'A': 'Alpha' }
              return (
                <div key={`${p.siswa_nisn}-${idx}`} className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500 shrink-0 overflow-hidden relative border border-slate-200">
                    <span className="absolute z-0">{getInitials(name)}</span>
                    <img 
                      src={`https://res.cloudinary.com/dwyhpysp5/image/upload/c_fill,w_100,h_100/SKL-BM/FOTO_${p.siswa_nisn}_${activeTa?.id}`} 
                      alt={name}
                      className="w-full h-full object-cover relative z-10"
                      onError={(e) => { e.target.style.display = 'none'; }}
                    />
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500">Kls {p.kelas}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${badges[p.status]}`}>
                      {labels[p.status]}
                    </span>
                    {p.status === 'T' && p.waktu && <div className="text-[10px] font-mono font-bold text-slate-400 mt-0.5">{p.waktu}</div>}
                  </div>
                </div>
              )
            })}
