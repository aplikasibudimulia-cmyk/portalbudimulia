...
export default function LaporanKeterlambatanSection() {
  ...
  const handleExportExcel = async () => {
    ...
    await downloadWorkbook(workbook, `Laporan_Keterlambatan_${startDate}_s.d_${endDate}.xlsx`)
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      {/* ── ULTRA-COMPACT HEADER BAR (Row 1) ─────────────────────────────────── */}
      <div className="bg-white px-3.5 py-2.5 rounded-xl border border-slate-200 shadow-2xs mb-2 flex flex-wrap items-center justify-between gap-2.5 print:hidden">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className="text-[11px] font-black text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
              {stats.totalKejadian || 0} Terlambat
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            ...
            title="Salin rekap keterlambatan untuk dibagikan ke grup WhatsApp"
          >
          ...
          <button
            ...
            className="px-2.5 py-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-lg text-xs font-bold flex items-center gap-1 transition-all shadow-2xs"
          ...
          <button
            ...
            className="px-2.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-xs font-bold flex items-center gap-1 transition-all shadow-2xs"
      ...
      {/* ── ULTRA-COMPACT TOOLBAR (Row 2) ────────────────────────────────────── */}
      <div className="bg-white px-3.5 py-2 rounded-xl border border-slate-200 shadow-2xs mb-3 flex flex-wrap items-center justify-between gap-2 print:hidden">
        {/* Preset Cepat + Tanggal Mulai & Selesai */}
        <div className="flex items-center gap-1.5 flex-wrap">
          {/* Preset Buttons */}
          <div className="flex items-center gap-0.5 bg-slate-100 p-0.5 rounded-lg text-xs">
            <button
              ...
              className="px-2 py-0.5 rounded-md font-bold text-slate-600 hover:text-slate-900 transition-all text-[11px]"
            >
              Hari Ini
            </button>
            <button
          ...
          <div className="flex items-center gap-1 text-xs">
            <input
              ...
              className="px-2 py-1 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer bg-slate-50"
            />
            <span className="text-slate-400 font-bold text-[11px]">s/d</span>
            <input
              type="date"
              value={endDate}
        ...
        {/* Filter Kelas & Live Search Siswa */}
        <div className="flex items-center gap-1.5 flex-1 justify-end max-w-lg">
          {/* Pilih Kelas */}
          <select
            value={selectedKelas}
            onChange={e => {
            ...
            className="px-2.5 py-1 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50 cursor-pointer shrink-0"
          >
          ...
          <div className="relative flex-1 min-w-[150px]" ref={dropdownRef}>
            <div className="relative">
              <input
                ...
                className={`w-full pl-2.5 pr-6 py-1 border rounded-lg text-xs font-medium outline-none transition-all ${
                  selectedSiswaObj
              ...
              {(studentSearchInput || selectedSiswaObj) ? (
                <button
                  ...
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 rounded-full p-0.5 transition-colors"
                >
                ...
                <svg className="w-3 h-3 text-slate-400 absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            ...
            {isDropdownOpen && autocompleteSuggestions.length > 0 && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl border border-slate-200 shadow-xl z-50 max-h-48 overflow-y-auto divide-y divide-slate-100 animate-fade-in">
                {autocompleteSuggestions.map(siswa => (
                  <button
                    key={siswa.nisn}
                    type="button"
                    onClick={() => handleSelectSiswa(siswa)}
                    className="w-full px-3 py-2 text-left hover:bg-indigo-50/70 transition-colors flex items-center justify-between group"
                  >
                    <div>
      ...
      {/* ── MINI STATS & ACTIVE FILTER BAR ─────────────────────────────────── */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-1 mb-2.5 print:hidden text-xs">
        <div className="flex items-center gap-1.5 flex-wrap">
          {selectedSiswaObj ? (
            <span className="flex items-center gap-1 text-indigo-700 font-bold bg-indigo-50 px-2.5 py-0.5 rounded-md border border-indigo-100 text-[11px]">
              <span>Filter Siswa:</span>
              <span>{selectedSiswaObj.nama_lengkap} ({selectedSiswaObj.kelas})</span>
            </span>
          ) : (
            <span className="text-slate-500 font-medium text-[11px]">
              Menampilkan: <strong className="text-slate-700">{selectedKelas === 'Semua' ? 'Semua Kelas' : `Kelas ${selectedKelas}`}</strong>
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap font-bold text-[11px]">
          <span className="px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200">
            Total: {stats.totalKejadian}x
          </span>
          <span className="px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-800 border border-indigo-200">
            Siswa: {stats.uniqueStudents} org
          </span>
          {stats.kelasTerbanyak !== '-' && (
            <span className="px-2 py-0.5 rounded-md bg-rose-50 text-rose-800 border border-rose-200">
              Top: Kelas {stats.kelasTerbanyak}
            </span>
          )}
          <button
            onClick={fetchLaporanKeterlambatan}
            disabled={loading}
            className="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md border border-slate-200 transition-all flex items-center gap-1 font-bold"
            title="Refresh Laporan Keterlambatan"
          >
