                      ...
                      <label key={f.id} className="flex items-start gap-3 cursor-pointer group">
                        <div className="mt-0.5 relative flex items-center justify-center">
                          ...
                          <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isChecked ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-300 group-hover:border-indigo-400'}`}>
                            {isChecked && <svg className="w-3.5 h-3.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                          </div>
                        </div>
                        <div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleAddRole} className="p-5 space-y-4">
              <div>
                ...
                <input type="text" required value={newRole.nama} onChange={e => setNewRole({...newRole, nama: e.target.value})} placeholder="Contoh: Wali Kelas" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Deskripsi (Opsional)</label>
                <textarea rows="2" value={newRole.deskripsi} onChange={e => setNewRole({...newRole, deskripsi: e.target.value})} placeholder="Penjelasan singkat tentang role ini" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm resize-none"></textarea>
              </div>
