...
function GuruAnnouncementSection({ type, students, fitur, fotos, onRefresh }) {
  ...
  const fetchFiles = async () => {
    ...
    // Batch processing for Guru (might have more students)
    for (let i = 0; i < kodes.length; i += 100) {
      const batch = kodes.slice(i, i + 100);
      const { data: eKode } = await supabase.from('enrollment').select('kode, nisn').in('kode', batch);
      const { data: eNisn } = await supabase.from('enrollment').select('kode, nisn').in('nisn', batch);
      if (eKode) enrData.push(...eKode);
  ...
  const handleToggleReq = async (nisn, reqId) => {
    ...
    if (upsertErr) {
      ...
      setFileReqs(prev => ({ ...prev, [nisn]: updatedReqs, [kode]: updatedReqs }))
    }
  ...
  const handleMassToggleReq = async (reqId, targetStatus) => {
    ...
    if (error) {
      ...
      for (const s of filteredStudents) {
        if (!files.has(s.nisn)) continue;
        newReqs[s.nisn] = { ...(fileReqs[s.nisn] || {}), [reqId]: targetStatus };
      }
  ...
  const handleToggleFileAccess = async (kodeParam, fileName) => {
    ...
    const { error } = await supabase.from('berkas_pengumuman')
      ...
      .match({ kode_siswa: kode, kode_jenis: type.dokumen_kode_jenis || type.kode_jenis })
    if (error) {
      alert('Gagal mengubah akses: ' + error.message)
    } else {
      setFileAccess(prev => ({ ...prev, [nisn]: !currentStatus }))
      const session = JSON.parse(localStorage.getItem('guru_session') || '{}')
      logActivity({
        userId: session.id,
...
export default function DashboardGuru() {
  ...
  const fetchData = async (userData) => {
    ...
    const allAssignedClasses = Array.from(new Set([
      ...allWaliAssignments.map(k => k.kelas),
      ...allMapelAssignments.map(m => m.kelas)
    ]))
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 relative">
        ...
        <div className={`flex-1 p-4 md:p-6 lg:p-8 flex flex-col ${activeMenu === 'dashboard' && session.roles.some(r => r.nama.toLowerCase().includes('piket')) && (!session.kelas || session.kelas.length === 0) && (!session.guru_mapel_raw || session.guru_mapel_raw.length === 0)
          ...
          <div className="w-full flex-1 flex flex-col min-h-0">
                  ...
                  {uniqueWaliClasses.length > 0 && (
                    <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
                      <button
                        ...
                        className={`whitespace-nowrap px-5 py-2 rounded-full text-sm font-medium transition-colors border ${siswaClassFilter === 'all'
                      ...
                      {uniqueWaliClasses.map(c => {
                        ...
                        return (
                          <button
                            ...
                            className={`whitespace-nowrap px-5 py-2 rounded-full text-sm font-medium transition-colors border ${siswaClassFilter === c
                                ? 'bg-indigo-600 text-white border-indigo-600 shadow-md'
                                : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 hover:border-slate-300'
                              }`}
                          >
            ...
            {menuTypes.map(t => {
              if (activeMenu === t.id) {
                const targetTaId = t.ta_referensi_id || activeTa?.id
                const targetWaliClasses = (session.kelas || [])
                  .filter(k => k.tahun_ajaran_id === targetTaId)
                  .map(k => k.kelas)

                const targetStudents = students.filter(s =>
                  s.tahun_ajaran_id === targetTaId && targetWaliClasses.includes(s.kelas)
                )
